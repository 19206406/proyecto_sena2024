...
@section('cabecera', 'Actualizar producto')

@section('contenido')
    <div class="flex justify-center my-6">
        <div class="card w-96 shadow-2xl bg-slate-50">
            <div class="card-body">
                {{-- Formulario para crear un producto --}}
                <form action="{{route('productos.update', $producto->id)}}" method="POST"> {{--Esto es para saber el id del producto a modificar--}}
                    @csrf
                    @method('PUT')
                    {{-- Nombre --}}
